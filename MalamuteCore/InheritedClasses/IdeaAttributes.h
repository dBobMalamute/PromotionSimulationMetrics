...
public:
    ...
    QList<QColor> labelsInColors() const;

    QList<int> dataTypesOut() const;
    QStringList labelsOut() const;
    QList<QColor> labelsOutColors() const;

    bool customLabels() const;
    bool customLabelColors() const;
    bool hasGear() const;
...
protected:
    ...
    /// The colors of the input label texts. Only used if m_customLabels = true;
    QList<QColor> m_labelsInColors;

    /// The data types of those plugs. Use magic<DataTypeAttributes>::cast().type();
    QList<int> m_dataTypesOut;
    /// The labels on the plugs. Only used if m_customLabels = true.
    QStringList m_labelsOut;
    /// The colors of the output label texts. Only used if m_customLabels = true;
    QList<QColor> m_labelsOutColors;

    /// Use custom labels. If this is false, the program uses the abbreviations.
    bool m_customLabels = false;
    /// Use custom label colors. If this is false, the program uses the default color.
    bool m_customLabelColors = false;
