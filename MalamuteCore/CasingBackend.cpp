...
void CasingBackend::onDisconnectPlugs(QList<UnplugWiresInfo> unplugWires)
{
    QJsonObject wiresToDelete;
    QJsonArray wiresJsonArray;

    for(int i = 0; i < unplugWires.length(); i++)
    {
        auto m = m_casingState.wires(unplugWires.at(0).isInput(), unplugWires.at(0).plugNumber());

        for(const auto& elem: m)
        {
            wiresJsonArray.append(elem.second->serialize());
        }
    }
    wiresToDelete["wires"] = wiresJsonArray;
    DeleteSelectionCommand* command = new DeleteSelectionCommand(wiresToDelete, m_casing->corkboard());
